...
class TurtleBotV0Env(gym.Env):
    ...
    def reset(self):
        ...
        #
        x_rand = np.array([0.1,0.2,0.3,0.7,0.8,0.9,0.48]) #First 4 are for trees, next 2 for rocks and the last for crafting table
        ...
        for i in range(self.n_trees): # Instantiate the trees
            self.x_pos.append(-self.width/2+self.width*x_rand[i]) # (Tree 1 will be at absolute location: -1.5 + 3*0.1 = -1.2)
            self.y_pos.append(-self.height/2+self.height*y_rand[i])
            self.map[int(self.width*10*x_rand[i])][int(self.height*10*y_rand[i])] = 1
        ...
        for i in range(self.n_rocks): # Instantiate the rocks
            ...
            self.map[int(self.width*10*x_rand[i + self.n_trees])][int(self.height*10*y_rand[i + self.n_trees])] = 2
        ...
        for i in range(self.n_table):
            if abs(-self.width/2+self.width*x_rand[i + self.n_trees + self.n_rocks]) < 0.3 and abs(-self.height/2+self.height*y_rand[i + self.n_trees + self.n_rocks]) < 0.3:
                ...
                self.map[int(self.width*10*x_rand[i + self.n_trees + self.n_rocks])][int(self.height*10*y_rand[i + self.n_trees + self.n_rocks])] = 3
    ...
    def step(self, action):
        ...
        self.map[int((self.agent_loc[0]+self.width/2)*10)][int((self.agent_loc[1]+self.height/2)*10)] = 0
        ...
        if action == 0: # Turn right
            ...
            if (abs(abs(x_new) - abs(self.width/2)) < 0.25) or (abs(abs(y_new) - abs(self.height/2)) < 0.25):
        ...
        obs = self.get_observation()
        self.map[int((self.agent_loc[0]+self.width/2)*10)][int((self.agent_loc[1]+self.height/2)*10)] = 567
